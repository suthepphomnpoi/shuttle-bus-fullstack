...
@section('content')
                ...
                <div class="modal-body">
                    <form id="placeForm">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="place_name" name="name" required
                                maxlength="100">
                        </div>
                    </form>
                </div>
    ...
    @push('scripts')
        ...
        <script>
            ...
            $(function() {
                ...
                const routeTable = $('#routeTable').DataTable({
                    ...
                    processing: true,
                    ajax: {
                        url: '{{ url('backoffice/routes/data') }}',
                        type: 'GET'
                    },
                    columns: [{
                            data: 'route_id',
        ...
        `
                ...
                $('#btnAddRoute').on('click', () => {
                    $('#route_id').val('');
                    $routeForm[0].reset();
                    $('#routeModalTitle').text('เพิ่มเส้นทาง');
                    routeModal.show();
                });
                $('#routeTable').on('click', '.btn-edit-route', function() {
                    const id = $(this).data('id');
                    $.get(`{{ url('backoffice/routes') }}/${id}`, res => {
                        $('#route_id').val(res.route_id);
                        $('#route_name').val(res.name);
                        $('#routeModalTitle').text('แก้ไขเส้นทาง');
                        routeModal.show();
                    });
                });
                $('#btnSaveRoute').on('click', function() {
                    ...
                    $.ajax({
                        url: id ? `{{ url('backoffice/routes') }}/${id}` :
                            `{{ url('backoffice/routes') }}`,
                        type: id ? 'PUT' : 'POST',
                        data: {
                            name: $('#route_name').val()
                        },
                ...
                $('#routeTable').on('click', '.btn-del-route', function() {
                    const id = $(this).data('id');
                    confirmSwal('ยืนยันการลบเส้นทาง?').then(res => {
                        if (!res.isConfirmed) return;
                        $.ajax({
                            url: `{{ url('backoffice/routes') }}/${id}`,
                            type: 'DELETE',
                            success: () => {
                                routeTable.ajax.reload(null, false);
                ...
                const placeTable = $('#placeTable').DataTable({
                    ...
                    processing: true,
                    ajax: {
                        url: '{{ url('backoffice/places/data') }}',
                        type: 'GET'
                    },
                    columns: [{
                            data: 'place_id',
          ...
          <button class="btn btn-sm btn-danger btn-del-place" data-id="${row.place_id}"><i class="ti ti-trash"></i></button>`
                ...
                $('#btnAddPlace').on('click', () => {
                    $('#place_id').val('');
                    $placeForm[0].reset();
                    $('#placeModalTitle').text('เพิ่มสถานที่');
                    placeModal.show();
                });
