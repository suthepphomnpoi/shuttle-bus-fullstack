                ...
                <div class="modal-body">
                    <form id="vehicleForm">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" min="1" class="form-control" id="v_capacity" name="capacity"
                                required>
                        </div>
                        <div class="mb-3">
        ...
        <script>
            ...
            $(function() {
                ...
                const vTable = $('#vehicleTable').DataTable({
                    ...
                    processing: true,
                    ajax: {
                        url: '{{ url('backoffice/vehicles/data') }}',
                        type: 'GET'
                    },
                    columns: [{
                            data: 'vehicle_id',
          ...
          <button class="btn btn-sm btn-danger btn-del-vehicle" data-id="${row.vehicle_id}"><i class="ti ti-trash"></i></button>`
                ...
                function loadTypes(selected) {
                    $.get(`{{ url('backoffice/vehicle-types/list') }}`, resp => {
                        const opts = (resp || []).map(r =>
                            `<option value="${r.vehicle_type_id}">${escapeHtml(r.name)}</option>`).join('');
                ...
                $vForm.validate({
                    ...
                    messages: {
                        ...
                        description: {
                            maxlength: 'ไม่เกิน 255 ตัวอักษร'
                        },
                        status: {
                ...
                $('#btnAddVehicle').on('click', () => {
                    $('#vehicle_id').val('');
                    $vForm[0].reset();
                    $('#vehicleModalTitle').text('เพิ่มรถ');
                    loadTypes();
                    vModal.show();
                });

                
                $('#vehicleTable').on('click', '.btn-edit-vehicle', function() {
                    const id = $(this).data('id');
                    $.get(`{{ url('backoffice/vehicles') }}/${id}`, res => {
                        $('#vehicle_id').val(res.vehicle_id);
                        $('#license_plate').val(res.license_plate);
                        loadTypes(res.vehicle_type_id);
                        $('#v_capacity').val(res.capacity || '');
                        $('#description').val(res.description || '');
                        $('#status').val(res.status);
                        $('#vehicleModalTitle').text('แก้ไขรถ');
                        vModal.show();
                    });
                });
                $('#btnSaveVehicle').on('click', function() {
                    if (!$vForm.valid()) return;
                    ...
                    const payload = {
                        ...
                        capacity: $('#v_capacity').val(),
                        description: $('#description').val(),
                        status: $('#status').val()
                    };
                    $.ajax({
                        url: id ? `{{ url('backoffice/vehicles') }}/${id}` :
                            `{{ url('backoffice/vehicles') }}`,
                        type: id ? 'PUT' : 'POST',
                        data: payload,
                        success: () => {
                            vModal.hide();
                ...
                $('#vehicleTable').on('click', '.btn-del-vehicle', function() {
                    const id = $(this).data('id');
                    confirmSwal('ยืนยันการลบรถ?').then(res => {
                        if (!res.isConfirmed) return;
                        $.ajax({
                            url: `{{ url('backoffice/vehicles') }}/${id}`,
                            type: 'DELETE',
                            success: () => {
                                vTable.ajax.reload(null, false);
                ...
                const tTable = $('#typeTable').DataTable({
                    ...
                    processing: true,
                    ajax: {
                        url: '{{ url('backoffice/vehicle-types/data') }}',
                        type: 'GET'
                    },
                    columns: [{
          ...
          <button class=\"btn btn-sm btn-danger btn-del-type\" data-id=\"${row.vehicle_type_id}\"><i class=\"ti ti-trash\"></i></button>`
                ...
                $tForm.validate({
                    ...
                    messages: {
                        name: {
                            required: 'กรุณากรอกชื่อประเภทรถ',
                ...
                $('#btnAddType').on('click', () => {
                    $('#vehicle_type_id_hidden').val('');
                    $tForm[0].reset();
                    $('#typeModalTitle').text('เพิ่มประเภทรถ');
                    tModal.show();
                });
                $('#typeTable').on('click', '.btn-edit-type', function() {

                    const id = $(this).data('id');


                    $.get(`{{ url('backoffice/vehicle-types') }}/${id}`, res => {
                        $('#vehicle_type_id_hidden').val(res.vehicle_type_id);
                        $('#type_name').val(res.name);
                        $('#typeModalTitle').text('แก้ไขประเภทรถ');
                        tModal.show();
                      });
                ...
                $('#btnSaveType').on('click', function() {
                    ...
                    if (!$tForm.valid()) return;

                    const id = $('#vehicle_type_id_hidden').val();
                    ...
                    $.ajax({
                        url: id ? `{{ url('backoffice/vehicle-types') }}/${id}` :
                            `{{ url('backoffice/vehicle-types') }}`,
                        type: id ? 'PUT' : 'POST',
                        data: payload,
                        success: () => {
                            tModal.hide();
                ...
                $('#typeTable').on('click', '.btn-del-type', function() {

                    const id = $(this).data('id');


                    confirmSwal('ยืนยันการลบประเภทรถ?').then(res => {

                        if (!res.isConfirmed) return;


                        $.ajax({
                            url: `{{ url('backoffice/vehicle-types') }}/${id}`,
                            type: 'DELETE',
                            success: () => {
                                tTable.ajax.reload(null, false);
                                showSwalSuccess('ลบสำเร็จ');
